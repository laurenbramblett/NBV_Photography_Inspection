function [newHits, newCoords, newangles] = rayTraceGP(pose, angles, obs, goal, cGoal, rad, maxRange)
    pt = pose;
    ...
    for a = 1:numAngles
        ...
        if isempty(goalSub)
            ...
            [goalInter, coordTermGoal] = checkIntersections(goalSub, pt, d(a, :), rad, d_norm(a));
        end
...
function [intersects, coordTerm] = checkIntersections(targets, pt, d, rad, d_norm)
    intersects = false;
    coordTerm = [0, 0];
    v = targets - pt;
    dv = v * d'; 
